@if($settings['logo'] ?? false)
    <img src="{{ Str::startsWith($settings['logo'], 'assets/') ? asset($settings['logo']) : Storage::url($settings['logo']) }}" {{ $attributes->merge(['class' => 'h-10 w-auto object-contain']) }} alt="MN Logo">
@else
