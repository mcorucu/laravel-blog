...
<article class="py-[15px] md:py-[35px] px-6 bg-mn-paper">
    <header class="max-w-4xl mx-auto text-center space-y-10 md:space-y-12 mb-16 md:mb-24">
        <div class="flex items-center justify-center space-x-4 animate-fade-in">
            <span class="text-[10px] uppercase tracking-[0.4em] font-bold text-mn-slate">{{ $post->category->name }}</span>
            <span class="w-1 h-1 bg-mn-silver rounded-full"></span>
            <span class="text-[10px] uppercase tracking-[0.4em] font-bold text-mn-slate lowercase italic tracking-normal">{{ $post->reading_time }} min read</span>
        </div>
        ...
        <div class="flex flex-col items-center space-y-4 pt-4 animate-fade-in delay-300">
            <div class="flex items-center space-x-3">
                ...
                <span class="text-[10px] md:text-xs uppercase tracking-[0.2em] font-bold">{{ $post->author_name }}</span>
                <div class="w-10 h-[1px] bg-mn-silver"></div>
            </div>
            <span class="text-[9px] md:text-[10px] uppercase tracking-[0.4em] text-mn-slate font-medium">{{ $post->published_at->format('M d, Y') }}</span>
        </div>
    </header>
    ...
    <div class="max-w-6xl mx-auto mb-32 animate-fade-in delay-500">
        <div class="aspect-[21/9] bg-mn-pearl/30 border border-mn-pearl/50 flex items-center justify-center overflow-hidden relative shadow-premium">
            @if($post->featured_image)
                <img src="{{ \Illuminate\Support\Facades\Storage::url($post->featured_image) }}" alt="{{ $post->title }}" class="w-full h-full object-cover hover:scale-105 transition-transform duration-[3s] ease-out">
            @else
                <div class="p-20 w-full h-full flex items-center justify-center">
                    <img src="/assets/illustrations/{{ $post->featured_svg }}" alt="{{ $post->title }}" class="w-full h-full object-contain hover:scale-105 transition-transform duration-[3s] ease-out">
                </div>
            @endif
        ...
        <div class="mt-20 md:mt-24 pt-12 md:pt-16 border-t border-mn-pearl/50 flex flex-col md:flex-row md:items-center justify-between space-y-12 md:space-y-0" x-data="{ 
            ...
            <div class="flex flex-col md:flex-row items-center md:space-x-8 space-y-6 md:space-y-0">
                ...
                <div class="flex items-center space-x-5">
                    <!-- X -->
                    <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($post->title) }}" target="_blank" class="w-10 h-10 md:w-8 md:h-8 flex items-center justify-center rounded-full border border-mn-pearl text-mn-slate hover:text-mn-black hover:border-mn-black transition-all duration-300 hover:-translate-y-1" title="Share on X">
                    ...
                    <!-- LinkedIn -->
                    <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}" target="_blank" class="w-10 h-10 md:w-8 md:h-8 flex items-center justify-center rounded-full border border-mn-pearl text-mn-slate hover:text-mn-black hover:border-mn-black transition-all duration-300 hover:-translate-y-1" title="Share on LinkedIn">
            ...
            <div class="flex items-center justify-center md:justify-end border-t border-mn-pearl/30 md:border-none pt-8 md:pt-0 w-full md:w-auto">
                <a href="/category/{{ $post->category->slug }}" class="group flex items-center space-x-3 text-[10px] uppercase tracking-[0.4em] font-bold text-mn-slate hover:text-mn-black transition-colors">
                    <span class="border-b border-transparent group-hover:border-mn-black pb-0.5 transition-all">Return to {{ $post->category->name }}</span>
